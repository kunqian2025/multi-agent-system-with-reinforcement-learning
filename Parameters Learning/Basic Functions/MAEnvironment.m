classdef MAEnvironment < handle
	properties (SetAccess = private)
        sizeMap = [400 400];%size of the map
		actionSpace = [1 2 3 4];
		locA = [100 100; 100 300];%default location of the agent
		locG = [200 200; 300 200];%location of the goal
		colorA = 'y';
		colorG = 'r';
        radii_A = 0.6;
		radii_G = 0.4;
	...
	methods
		...
		function [next_state, reward, done] = step(obj,action) 
            ...
            switch action(1)
				case 1
					next_state(1,1) = next_state(1,1) + 1*obj.agentSpeed;
				case 2
					next_state(1,2) = next_state(1,2) + 1*obj.agentSpeed;
				case 3
					next_state(1,1) = next_state(1,1) - 1*obj.agentSpeed;
				case 4
					next_state(1,2) = next_state(1,2) - 1*obj.agentSpeed;
            ...
            switch action(2)
				case 1
					next_state(2,1) = next_state(2,1) + 1*obj.agentSpeed;
				case 2
					next_state(2,2) = next_state(2,2) + 1*obj.agentSpeed;
				case 3
					next_state(2,1) = next_state(2,1) - 1*obj.agentSpeed;
				case 4
					next_state(2,2) = next_state(2,2) - 1*obj.agentSpeed;
...
%             end
            ...
            if (pdist([next_state(1,:); obj.locG(1,:)]) < 5 && ...
                    pdist([next_state(2,:); obj.locG(2,:)]) < 5) || ...
                    (pdist([next_state(1,:); obj.locG(2,:)]) < 5 && ...
                    pdist([next_state(2,:); obj.locG(1,:)]) < 5)
                reward = obj.rwdG;
				done = true; %find the goal
            end
            
            %to check if collide
            if pdist([next_state(1,:); next_state(2,:)]) < 10
                reward = obj.rwdC;
